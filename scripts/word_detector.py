...
bert_model = TFBertForTokenClassification.from_pretrained("bert-base-uncased", num_labels=3)  # 0=OK, 1=ANACHRONISM, 2=STYLE_MISMATCH
...
for labels in train_labels:
    padded = [0] + labels + [0] * (max_length - len(labels) - 1)  # [CLS] + labels + padding
    padded_labels.append(padded[:max_length])

train_dataset = tf.data.Dataset.from_tensor_slices((dict(train_encodings), padded_labels)).batch(2)
...
bert_model.save_pretrained("models/bert_trained")  # Save after training
...
def suggest_alternative(word, timeframe):
    ...
    if word not in word2vec:
        return "unknown"
    # Specific rule for "lit" in 20th century
    if timeframe == "20th" and word == "lit":
        return "great"  # Hardcode "great" as a 20th-century formal replacement
    # Prioritize Word2Vec
...
def analyze_file(file_path, timeframe, expected_style="neutral"):
    ...
    valid_words = [token.text for token in doc if token.text in word2vec and token.text in anachronism_dict]  # Only dictionary words
    if valid_words:
        ...
        for token in doc:
            ...
            if word in word2vec and word in anachronism_dict and word not in seen_words:
                ...
                if similarity < 0.7:  # Raised threshold
                    year = anachronism_dict.get(word, {}).get("year", 0)
